...
Installed release builds check for an update in the background when the app opens, including when automatic downloads are disabled. **Auto-download** controls background downloading; manual checks leave downloading to the **Download** action. **Auto-install** safely stops and neutralizes the receiver before starting a verified downloaded update. If receiver shutdown cannot be confirmed, the installer does not start and DS Controller requires a restart before another update or receiver attempt. Network failures do not stop the receiver. **Later** discards the verified download and suppresses automatic rediscovery until a manual check or the next app session. The desktop updater does not update the DS ROM or `ds-controller.ini`.

Windows uses the standard current-user NSIS updater and relaunch behavior. Ubuntu uses Tauri's Debian updater path and may request administrator authentication. If a Debian installation returns an error, DS Controller conservatively disables receiver starts until the app is restarted and the package state is checked. If an installation or receiver-preparation task panics, the app leaves its busy state and allows closing, while keeping receiver starts blocked until restart because safe completion is unconfirmed.
