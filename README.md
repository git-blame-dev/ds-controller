...
Installed release builds check for an update in the background when the app opens, including when automatic downloads are disabled. **Auto-download** controls background downloading; manual checks leave downloading to the **Download** action. **Auto-install** starts a verified downloaded update only when the receiver is already stopped, and never stops an active receiver automatically. Otherwise the update remains ready for **Restart and update** or **Later**. Network failures do not stop the receiver. **Later** discards the verified download and suppresses automatic rediscovery until a manual check or the next app session. The desktop updater does not update the DS ROM or `ds-controller.ini`.
