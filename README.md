...
Installed release builds check for an update in the background when the app opens, including when automatic downloads are disabled. **Automatically download updates when the app opens** controls downloading only; manual checks leave downloading to the **Download** action. Network failures do not stop the receiver. Installation begins only after **Restart and update** is selected. **Later** discards the verified download and suppresses automatic rediscovery until a manual check or the next app session. The desktop updater does not update the DS ROM or `ds-controller.ini`.
