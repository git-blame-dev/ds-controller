.SUFFIXES:

WINDOWS_TARGET := x86_64-pc-windows-msvc
DEVKITARM_IMAGE ?= devkitpro/devkitarm:20260221@sha256:4debd5b33cf4361a557b6bf3be5ff823804868125ce1429912f1a4e773e7ac5d
DOCKER_USER := $(shell id -u):$(shell id -g)
PC_CARGO_TARGET_DIR ?= $(CURDIR)/pc/target
PC_CARGO_RELEASE_DIR := $(PC_CARGO_TARGET_DIR)/$(WINDOWS_TARGET)/release
WINDOWS_NSIS_BUNDLE_DIR := $(PC_CARGO_RELEASE_DIR)/bundle/nsis
LINUX_DEB_BUNDLE_DIR := $(PC_CARGO_TARGET_DIR)/release/bundle/deb
LINUX_DEB_ARTIFACT := $(CURDIR)/dist/linux/ds-controller-linux-amd64.deb
LINUX_PACKAGE_VERSION ?= 0.1.0
PC_STAGED_RELEASE_DIR := $(CURDIR)/dist/pc
NDS_STAGED_RELEASE_DIR := $(CURDIR)/dist/nds
PC_PORT ?= 26760

-include build.mk

PC_IP ?= 192.0.2.1

.DEFAULT_GOAL := help

.PHONY: help all nds nds-local pc pc-dist linux linux-dist linux-verify nds-dist dist app-dev test test-release-helpers test-ci-package-prereqs clean

help:
	@printf '%s\n' 'Targets:'
	@printf '%s\n' '  make nds       Build the Nintendo DS ROM in Docker; PC_IP/PC_PORT from build.mk are optional defaults'
	@printf '%s\n' '  make nds-local Build the Nintendo DS ROM with local devkitPro'
	@printf '%s\n' '  make pc        Cross-build the unsigned current-user Windows NSIS installer from Linux'
	@printf '%s\n' '  make pc-dist   Build and stage the Windows installer under dist/pc'
	@printf '%s\n' '  make linux     Build the Ubuntu/Debian Tauri GUI package'
	@printf '%s\n' '  make linux-dist Build and stage the Debian package under dist/linux'
	@printf '%s\n' '  make linux-verify Verify the staged Debian package contents and metadata'
	@printf '%s\n' '  make nds-dist  Build and stage NDS files under dist/nds'
	@printf '%s\n' '  make dist      Build and stage all release files under dist/'
	@printf '%s\n' '  make app-dev   Run the Tauri GUI app in development mode'
	@printf '%s\n' '  make test      Run DS host tests, receiver Rust tests, and frontend build'
	@printf '%s\n' '  make all       Build all platform release artifacts'

all: nds pc linux

nds:
	docker run --rm --user $(DOCKER_USER) -e HOME=/tmp -v "$(CURDIR)":/workspace -w /workspace \
	$(DEVKITARM_IMAGE) \
	$(MAKE) nds-local PC_IP="$(PC_IP)" PC_PORT="$(PC_PORT)"
	@printf 'NDS ROM: %s\n' "$(CURDIR)/nds/build/ds-controller.nds"

nds-local:
	$(MAKE) -C nds clean
	$(MAKE) -C nds PC_IP="$(PC_IP)" PC_PORT="$(PC_PORT)"
	@printf 'NDS ROM: %s\n' "$(CURDIR)/nds/build/ds-controller.nds"

pc:
	@command -v cargo-xwin >/dev/null || { printf '%s\n' 'Missing cargo-xwin. Install with: cargo install cargo-xwin' >&2; exit 1; }
	@command -v llvm-rc >/dev/null || { printf '%s\n' 'Missing llvm-rc. Install LLVM tools before running make pc.' >&2; exit 1; }
	@command -v lld-link >/dev/null || { printf '%s\n' 'Missing lld-link. Install lld before running make pc.' >&2; exit 1; }
	rm -rf "$(WINDOWS_NSIS_BUNDLE_DIR)"
	CARGO_TARGET_DIR="$(PC_CARGO_TARGET_DIR)" pnpm --dir pc/app tauri build --runner cargo-xwin --target $(WINDOWS_TARGET) --bundles nsis
	@printf 'Windows NSIS package directory: %s\n' "$(WINDOWS_NSIS_BUNDLE_DIR)"

pc-dist: pc
	@mkdir -p "$(PC_STAGED_RELEASE_DIR)"
	@set -- "$(WINDOWS_NSIS_BUNDLE_DIR)"/*.exe; \
	if [ "$$#" -ne 1 ] || [ ! -f "$$1" ]; then \
		printf '%s\n' 'Expected exactly one NSIS installer from the Windows build' >&2; \
		exit 1; \
	fi; \
	cp "$$1" "$(PC_STAGED_RELEASE_DIR)/ds-controller-windows-x86_64-setup.exe"
	@printf 'Staged Windows files: %s\n' "$(PC_STAGED_RELEASE_DIR)"

linux:
	rm -rf "$(LINUX_DEB_BUNDLE_DIR)"
	CARGO_TARGET_DIR="$(PC_CARGO_TARGET_DIR)" pnpm --dir pc/app tauri build --bundles deb --config '{"version":"$(LINUX_PACKAGE_VERSION)"}'
	@printf 'Ubuntu/Debian package directory: %s\n' "$(LINUX_DEB_BUNDLE_DIR)"

linux-dist: linux
	@mkdir -p "$(dir $(LINUX_DEB_ARTIFACT))"
	@set -- "$(LINUX_DEB_BUNDLE_DIR)"/*.deb; \
	if [ "$$#" -ne 1 ] || [ ! -f "$$1" ]; then \
		printf '%s\n' 'Expected exactly one Debian package from the Linux build' >&2; \
		exit 1; \
	fi; \
	cp "$$1" "$(LINUX_DEB_ARTIFACT)"
	@printf 'Staged Ubuntu/Debian package: %s\n' "$(LINUX_DEB_ARTIFACT)"

linux-verify: linux-dist
	sh scripts/verify-linux-deb.sh "$(LINUX_DEB_ARTIFACT)" "$(LINUX_PACKAGE_VERSION)"

nds-dist: nds
	@mkdir -p "$(NDS_STAGED_RELEASE_DIR)"
	@cp "$(CURDIR)/nds/build/ds-controller.nds" "$(NDS_STAGED_RELEASE_DIR)/ds-controller.nds"
	@cp "$(CURDIR)/nds/ds-controller.ini" "$(NDS_STAGED_RELEASE_DIR)/ds-controller.ini"
	@printf 'Staged NDS files: %s\n' "$(NDS_STAGED_RELEASE_DIR)"

dist: pc-dist linux-dist nds-dist

app-dev:
	pnpm --dir pc/app tauri dev

test:
	$(MAKE) -C nds test
	$(MAKE) test-release-helpers
	cargo fmt --all --check
	cargo test --manifest-path Cargo.toml
	cargo clippy --manifest-path Cargo.toml --workspace --all-targets -- -D warnings
	pnpm --dir pc/app lint
	pnpm --dir pc/app test
	pnpm --dir pc/app build

test-release-helpers:
	bash scripts/tests/release-helpers-test.sh

test-ci-package-prereqs:
	$(MAKE) -C nds test
	$(MAKE) test-release-helpers
	cargo fmt --all --check
	cargo test --manifest-path Cargo.toml
	cargo clippy --manifest-path Cargo.toml --workspace --all-targets -- -D warnings
	pnpm --dir pc/app lint
	pnpm --dir pc/app test

clean:
	$(MAKE) -C nds clean
	cargo clean
	rm -rf "$(CURDIR)/dist" "$(CURDIR)/package" "$(CURDIR)/pc/target" "$(CURDIR)/pc/app/dist" "$(CURDIR)/pc/app/.vite" "$(CURDIR)/pc/app/src-tauri/gen"
